...
const Cart = ({ match, location, history }) => {
  ...
  // Add the items to the state & localStorage
  useEffect(() => {
    productId && dispatch(addItemToCart(productId, qty));
  }, [dispatch, productId, qty]);

  // Send the user to enter shipping info on checkout
  ...
  return (
    <div
      className={`w-full h-full px-4 sm:px-12 my-7 mx-auto flex flex-col self-center`}
    >
      {items.length === 0 ? (
        <Message
          info="Your cart is empty. Go"
          link={
            <Link
              className={`ml-1 text-2xl transform duration-300 hover:scale-105 
                          hover:translate-x-1 bg-clip-text text-transparent 
            ...
            {items.map((item, index) => {
              return (
                <Item
                  ...
                  removeItem={() => dispatch(removeItemFromCart(item.product))}
                />
              );
            })}

            <div className="bg-footer rounded-b-xl h-14 w-full border-t border-gray-700" />
          </div>
            ...
            <div className="flex flex-col">
              <div className="grid grid-cols-2">
                <div className="flex flex-row justify-center bg-gray-700 p-4 border-r border-gray-600">
                  <p className="text-right font-semibold text-white mr-2">
                    Items:
                  </p>
