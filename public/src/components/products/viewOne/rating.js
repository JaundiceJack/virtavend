...
const Rating = ({ value, numReviews }) => {
  ...
  return (
    <div
      className={`flex flex-row items-center py-4 px-6 bg-gray-800 border-r 
                  border-gray-600 border-b border-t border-gray-600 font-semibold `}
    >
      ...
      <h3 className={"font-semibold text-lg whitespace-nowrap text-white "}>
        {[0, "", null, undefined].indexOf(numReviews) !== -1
          ? "No Reviews"
          : numReviews === 1
          ? numReviews + " Review"
          : numReviews > 1
