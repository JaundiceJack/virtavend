...
const Product = ({ match, history }) => {
  ...
  // Send the user to their cart upon adding a product
  const addToCart = () => {
    history.push(`/cart/${match.params.id}?qty=${qty}`);
  };

  // Get the requested product on page load
  const dispatch = useDispatch();
  useEffect(() => dispatch(getProduct(match.params.id)), [dispatch, match]);

  return (
    <div
      className={`w-full h-full px-4 sm:px-12 my-7 mx-auto
    ...
    >
      <div className={`w-full h-full flex items-center justify-center`}>
        {loading ? (
          ...
          <div
            className={`grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 2xl:mt-7`}
          >
            {/* Product Info */}
            <div
              className={`flex flex-col col-span-1 md:col-span-2 mb-8 2xl:mb-0`}
            >
              <div
                className={`grid grid-cols-1 sm:grid-cols-2 rounded-b-lg rounded-t-xl bg-gray-700`}
              >
            ...
            {/* Product Actions */}
            <div
              className={`w-full col-span-2 2xl:col-span-1 md:w-3/4
                          3xl:w-3/5 self-start justify-self-center`}
            >
              <Price price={product.price} />
              <div
                className={`h-px w-full bg-gradient-to-r from-gray-700 via-gray-600 to-gray-700`}
              />
